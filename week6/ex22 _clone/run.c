...
Message *runCommand (int sock, char *input_str) {
    ...
    if(cmd->opcode == USER) return runUSER(sock,cmd->id);
    else if(cmd->opcode == PASS) return runPASS(sock,cmd->pass);
    else if(cmd->opcode == LOUT) return runLOUT(sock,cmd->id,cmd->pass);
    else return NULL;
}
